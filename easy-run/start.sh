...
set -- dbvtk-*.war
DBVTK_WAR="$1"
if [[ $DBVTK_WAR == "dbvtk-*.war"        ]]; then echo "Could not find dbvtk war file"; exit; fi
set -- solr-*.zip
SOLR_ZIP="$1"
if [[ $SOLR_ZIP  == "solr-*.zip"         ]]; then echo "Could not find solr zip file";  exit; fi
...
echo "The Database Visualization Toolkit will be available at: http://127.0.0.1:$webPort"
echo
echo "To add databases use the following command:"
echo "java -jar \"-Dfile.encoding=UTF-8\" \"$DIR/$DBPTK_JAR\" -i siard-2 -if path/to/siard/file -e solr -ep $solrCloudPort -ezp $solrZooPort"
