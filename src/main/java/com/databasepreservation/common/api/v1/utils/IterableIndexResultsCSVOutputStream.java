  ...
  public void consumeOutputStream(final OutputStream out) throws IOException {
    ...
    for (ViewerRow row : results) {
      if (isFirst) {
        printer = getFormat()
          .withHeader(
            configTable.getCSVHeaders(fieldsToReturn, exportDescription).toArray(new String[0]))
          .print(writer);
        isFirst = false;
      }

      printer.printRecord(HandlebarsUtils.getRowValues(row, configTable, fieldsToReturn));
