...
public class UserLoginHelper {
  ...
  public static User casLogin(final String username, final HttpServletRequest request) {
    ...
    if (request.getUserPrincipal() instanceof AttributePrincipal) {
      ...
      final List<String> rolesConfigurationValue = ViewerConfiguration.getInstance().getViewerConfigurationAsList(ViewerConfiguration.PROPERTY_AUTHORIZATION_ROLES_ATTRIBUTE);
      ...
      for (String rolesAttribute : rolesConfigurationValue) {
        if (attributes.get(rolesAttribute) instanceof String) {
          ...
          mapCasAttributeList(user, attributes, rolesAttribute, (user1, allRoles) -> user1.getAllRoles().addAll(allRoles));
          mapCasAttributeList(user, attributes, rolesAttribute, (user1, directRoles) -> user1.getDirectRoles().addAll(directRoles));
        }
      }
  ...
  private static void mapAuthorizedGroups(User user, Set<AuthorizationGroup> authorizationGroups) {
    ...
    for (AuthorizationGroup group : authorizationGroups) {
      List<String> rolesAttribute = ViewerConfiguration.getInstance().getViewerConfigurationAsList(ViewerConfiguration.PROPERTY_AUTHORIZATION_ROLES_ATTRIBUTE);
